...
// Local navigation links for current page
const localSections = [
  { id: "home", label: "Intro" },
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    // Get all sections
    const sections = localSections.map(section => ({
      id: section.id,
    ...
    // Create observer with better threshold for each section
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (bestMatch && bestMatch !== activeSection) {
          setActiveSection(bestMatch);
        }
      },
      { 
        threshold: [0, 0.25, 0.5, 0.75, 1],
        rootMargin: "-20% 0px -30% 0px" // Better margin for detection
      }
    );
  ...
  // Navigation handler
  const handleNavigation = useCallback((href, isSection = false) => {
    setIsOpen(false);
  ...
  const bgColorClass = useMemo(() => {
    return isHomePage ? "text-white" : "text-[#1a1a1a]";
  }, [isHomePage]);

  // Memoize menu button styles
  const menuButtonClass = useMemo(() => {
    return `text-sm font-semibold uppercase tracking-widest flex items-center gap-3 hover:opacity-70 transition-opacity group cursor-none ${bgColorClass}`;
  }, [bgColorClass]);
    ...
    return (
      <div className="fixed right-6 md:right-10 top-1/2 -translate-y-1/2 flex flex-col items-end gap-6 z-40 pointer-events-none hidden md:flex">
        {localSections.map((section) => (
          <button
            key={section.id}
            onClick={() => handleNavigation(`#${section.id}`, true)}
  ...
  }, [isHomePage, activeSection, handleNavigation]);

  return (
    <>
      {/* Top Navigation Bar */}
      <div className={`fixed top-0 left-0 w-full p-6 md:p-10 flex justify-between items-center z-50 pointer-events-none transition-colors duration-300 ${bgColorClass}`}>
        ...
        >
          ...
          <div className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all ${
            isHomePage ? "border-white group-hover:border-white/70" : "border-[#1a1a1a] group-hover:border-[#1a1a1a]/70"
          }`}>
            <motion.div 
                    ...
                    return (
                      <div key={page.name} className="overflow-hidden">
                        <motion.li
                          ...
                          animate={{ y: "0%" }}
                          exit={{ y: "100%" }}
                          transition={{ duration: 0.6, delay: 0.2 + (i * 0.08), ease: [0.76, 0, 0.24, 1] }}
                        >
                          <button
                            onClick={() => handleNavigation(page.href, false)}
                            className={`group flex items-center text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-none transition-colors duration-500 cursor-none w-full text-left ${
                              isCurrentPage 
                  ...
                  <ul className="flex flex-col space-y-2">
                    {socials.map((social) => (
                      <motion.li
                        ...
                        initial={{ x: -20, opacity: 0 }}
                        animate={{ x: 0, opacity: 1 }}
                        transition={{ delay: social.delay - 0.1 }}
                      >
                        <a 
                          href={social.href} 
                          target="_blank"
