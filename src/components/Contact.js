...
function SocialSection() {
  // Memoize social link hover colors
  const getLinkColor = useCallback((name) => {
    if (name === "LinkedIn") return "group-hover:text-[#3B82F6]";
    if (name === "GitHub") return "group-hover:text-[#F97316]";
    return "group-hover:text-[#3B82F6]";
  }, []);

  // Memoize social link opacity color
  const getLinkOpacityColor = useCallback((name) => {
    if (name === "LinkedIn") return "group-hover:text-[#3B82F6]";
    if (name === "GitHub") return "group-hover:text-[#F97316]";
    return "group-hover:text-[#3B82F6]";
  }, []);
      ...
      <div>
        ...
        <div className="flex flex-col">
          {socialLinks.map((link) => (
            ...
            >
              <span className={`text-2xl lg:text-3xl font-black uppercase tracking-tighter transition-colors ${getLinkColor(link.name)}`}>
                {link.name}
              </span>
              <span className={`text-xs font-mono opacity-20 group-hover:opacity-100 transition-opacity ${getLinkOpacityColor(link.name)}`}>
                LINK_{link.id} ↗
              </span>
            </motion.a>
          ))}
