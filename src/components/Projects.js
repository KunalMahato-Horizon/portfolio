...
export default function Projects() {
  ...
  const totalCards = projects.length + 1; // +1 for the "View All" card
  const cardWidth = isMobile ? 85 : 50; // vw per card
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 h-screen flex items-center overflow-hidden">
        ...
        {/* Massive Dynamic Background Text - Hidden on mobile for performance */}
        {!isMobile && (
        ...
        >
          {projects.map((project, index) => (
            <ProjectCard key={project.id} project={project} progress={scrollYProgress} index={index} isMobile={isMobile} />
          ))}
          
          {/* View All Projects Card */}
          <ViewAllCard projects={projects} scrollYProgress={scrollYProgress} isMobile={isMobile} />
        </motion.div>
        ...
        <div className="absolute bottom-6 md:bottom-12 left-4 md:left-6 right-4 md:right-12 flex justify-between items-end z-30 pointer-events-none">
          <div className="flex flex-col text-[#1a1a1a]">
            <span className="text-[8px] md:text-[10px] font-mono uppercase tracking-[0.2em] md:tracking-[0.3em] opacity-50 mb-1 md:mb-2">
              {isMobile ? "Scroll" : "Scroll / Explore"}
            </span>
          </div>
        ...
        {/* Floating Portfolio Link - Hidden on mobile */}
        {!isMobile && (
          <motion.div 
...
function ProjectCard({ project, progress, index, isMobile }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-[10%] md:top-[15%] lg:top-[20%] left-0 md:-left-8 lg:-left-12 z-20 pointer-events-none px-4 md:px-0">
        ...
        <h3 className="text-4xl md:text-6xl lg:text-8xl xl:text-[8vw] font-black tracking-tighter uppercase leading-[0.9] md:leading-[0.85] text-[#1a1a1a] drop-shadow-2xl mix-blend-difference text-white">
      ...
      <div className="relative w-full aspect-[4/3] md:aspect-[16/10] overflow-hidden rounded-sm bg-[#dcdcdc] border border-[#1a1a1a]/5 shadow-2xl z-10 mt-8 md:mt-12">
        <motion.img
          style={!isMobile ? { x: imageX, scale: 1.15 } : {}}
          src={project.img}
          alt={project.title}
        ...
        {/* Live Link Overlay - Touch friendly for mobile */}
        <motion.div 
          initial={{ opacity: 0 }}
        ...
        >
          <motion.a
            ...
            className={`px-4 md:px-5 lg:px-6 py-2 md:py-2.5 lg:py-3 text-xs md:text-sm font-bold uppercase tracking-wider md:tracking-widest rounded-full transition-all duration-300 cursor-none ${
              project.id === "01" ? "bg-[#3B82F6] hover:bg-[#3B82F6]/80 active:bg-[#3B82F6]/90" :
              project.id === "02" ? "bg-[#F97316] hover:bg-[#F97316]/80 active:bg-[#F97316]/90" :
              "bg-white hover:bg-[#e3e3e3] active:bg-[#d4d4d4] text-[#1a1a1a]"
            } text-white`}
      ...
      <div className="relative z-20 flex justify-between items-center w-full mt-4 md:mt-6 lg:mt-8">
        <motion.a
          ...
          className={`group/live flex items-center gap-2 md:gap-3 text-[8px] md:text-[10px] lg:text-xs font-bold uppercase tracking-[0.15em] md:tracking-[0.2em] transition-all duration-300 pointer-events-auto ${
            project.id === "01" ? "text-[#3B82F6] active:text-[#3B82F6]/70" :
            project.id === "02" ? "text-[#F97316] active:text-[#F97316]/70" :
            "text-[#1a1a1a] active:text-[#1a1a1a]/70"
          }`}
        >
...
function ViewAllCard({ projects, scrollYProgress, isMobile }) {
  return (
    <div className="group relative flex flex-col justify-center w-[85vw] md:w-[60vw] lg:w-[50vw] h-screen flex-shrink-0">
          ...
          >
            <div className="flex flex-col items-center gap-2 md:gap-4 p-4">
              <span className="px-4 md:px-5 lg:px-6 py-2 md:py-2.5 lg:py-3 bg-white text-[#1a1a1a] text-xs md:text-sm font-bold uppercase tracking-wider md:tracking-widest rounded-full active:scale-95 transition-transform">
                {isMobile ? "Explore →" : "View Full Portfolio →"}
              </span>
      ...
      <div className="relative z-20 flex justify-end w-full mt-4 md:mt-6 lg:mt-8">
        ...
        >
          {isMobile ? "All Work" : "Explore All Work"}
